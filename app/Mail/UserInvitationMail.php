...
/**
 * Email d'invitation avec système de {BALISES}.
 *
 * Balises disponibles dans le template :
 * ─────────────────────────────────────────────────────────────
 * {CHER_E}            → Cher / Chère / Dear / Estimado(a) …
 * {SALUTATION}        → Monsieur / Madame / Mr. / Ms. …
 * {PRENOM}            → Prénom (premier mot du nom)
 * {NOM_COMPLET}       → Nom complet
 * {EMAIL}             → Adresse email
 * {LIEN_ACTIVATION}   → URL d'activation (lien cliquable)
 * {NOM_ENTREPRISE}    → Credixa Invest
 * {FORMULE_POLITESSE} → Cordialement / Best regards …
 * {EQUIPE}            → L'équipe Credixa Invest / The Credixa Invest Team …
 * {NOTICE_PERSONNEL}  → "Ce lien est personnel et unique…"
 * {NOTICE_IGNORE}     → "Si vous n'êtes pas à l'origine…"
 * ─────────────────────────────────────────────────────────────
 */
class UserInvitationMail extends Mailable
{
    ...
    // ── Template par défaut (éditable, peut venir de la DB) ──────────────────
    // Les {BALISES} sont détectées et remplacées automatiquement
    // selon la langue (locale) et le genre (gender) du destinataire.
    private const DEFAULT_TEMPLATE = <<<'TPL'
{CHER_E} {SALUTATION} {NOM_COMPLET},
...
TPL;
    ...
    // ── Traductions du corps selon la locale ─────────────────────────────────
    private const BODY = [
        'fr' => [
    ...
    private const TAGS = [
        ...
        '{NOTICE_IGNORE}' => [
            ...
            'en' => ['M' => 'If you did not request this account, you can safely ignore this email.',
                     'F' => 'If you did not request this account, you can safely ignore this email.',
                     'N' => 'If you did not request this account, you can safely ignore this email.'],
            'es' => ['M' => 'Si usted no solicitó la creación de esta cuenta, puede ignorar este correo.',
                     'F' => 'Si usted no solicitó la creación de esta cuenta, puede ignorar este correo.',
                     'N' => 'Si usted no solicitó la creación de esta cuenta, puede ignorar este correo.'],
            'pl' => ['M' => 'Jeśli nie prosiłeś o utworzenie tego konta, możesz zignorować ten e-mail.',
                     'F' => 'Jeśli nie prosiłaś o utworzenie tego konta, możesz zignorować ten e-mail.',
                     'N' => 'Jeśli nie prosiłeś/aś o utworzenie tego konta, możesz zignorować ten e-mail.'],
        ],
    ];

    // ── Sujets selon la locale ───────────────────────────────────────────────
    ...
    public function content(): Content
    {
        ...
        // ── Résolution des balises statiques ────────────────────────────────
        $resolved = [];
        ...
        // ── Résolution du corps selon locale ────────────────────────────────
        ...
        // ── Nettoyage : "{SALUTATION} " → " " si neutre ─────────────────────
        $greeting = trim($resolved['{CHER_E}'] . ' ' . $resolved['{SALUTATION}'] . ' ' . $this->user->name);

        // ── Application du template ──────────────────────────────────────────
        $body = $this->applyReplacements(self::DEFAULT_TEMPLATE, $resolved);
    ...
    /** Remplace toutes les {BALISES} dans $text par les valeurs résolues */
