<?php

return [
    'success_contact' => 'WiadomoÅ›Ä‡ zostaÅ‚a wysÅ‚ana pomyÅ›lnie',
    'success_sbscribe' => 'Subskrypcja zakoÅ„czona pomyÅ›lnie',
    'error' => 'WystÄ…piÅ‚ bÅ‚Ä…d podczas wysyÅ‚ania. SprÃ³buj ponownie pÃ³Åºniej.',
    'success_loan' => 'TwÃ³j wniosek kredytowy zostaÅ‚ wysÅ‚any pomyÅ›lnie. Odpowiemy tak szybko, jak to moÅ¼liwe.',
    'error_loan' => 'WystÄ…piÅ‚ bÅ‚Ä…d podczas wysyÅ‚ania wniosku kredytowego. SprÃ³buj ponownie pÃ³Åºniej.',

    // Emaile dotyczÄ…ce wniosku kredytowego
    'months'               => 'miesiÄ™cy',
    'month_abbr'           => 'mies.',
    'optional'             => 'opcjonalne',
    'loan_admin_subject'   => 'Nowy wniosek kredytowy',
    'loan_admin_intro'     => 'Klient wÅ‚aÅ›nie przesÅ‚aÅ‚ wniosek kredytowy za poÅ›rednictwem strony Credixa Invest.',

    'loan_confirm_subject'   => 'TwÃ³j wniosek kredytowy jest przetwarzany',
    'loan_confirm_greeting'  => 'Witaj :name,',
    'loan_confirm_body'      => 'OtrzymaliÅ›my TwÃ³j wniosek kredytowy na kwotÄ™ :amount :currency na okres :duration miesiÄ™cy. Jest on obecnie przetwarzany przez nasz zespÃ³Å‚.',
    'loan_confirm_footer'    => 'Skontaktujemy siÄ™ z TobÄ… tak szybko, jak to moÅ¼liwe. DziÄ™kujemy za zaufanie.',
    'loan_confirm_signature' => 'ZespÃ³Å‚ Credixa Invest',
    'loan_confirm_noreply'   => 'Ta wiadomoÅ›Ä‡ zostaÅ‚a wysÅ‚ana z adresu no-reply. Prosimy nie odpowiadaÄ‡ bezpoÅ›rednio na tÄ™ wiadomoÅ›Ä‡.',

    'loan_conditions_title'  => 'Warunki kwalifikowalnoÅ›ci',
    'loan_conditions_text'   => 'Aby uzyskaÄ‡ poÅ¼yczkÄ™, naleÅ¼y mieÄ‡ ukoÅ„czone 18 lat, posiadaÄ‡ stabilne miesiÄ™czne dochody i byÄ‡ w stanie spÅ‚acaÄ‡ zgodnie z ustalonymi warunkami.',
    'loan_complete_btn'      => 'UzupeÅ‚nij wniosek',
    'loan_complete_intro'    => 'Aby sfinalizowaÄ‡ wniosek, kliknij poniÅ¼szy przycisk i przeÅ›lij swÃ³j peÅ‚ny adres oraz kopiÄ™ dokumentu toÅ¼samoÅ›ci.',

    'docs_subject'   => 'Dokumenty â€” Wniosek kredytowy',
    'docs_intro'     => 'Klient przesÅ‚aÅ‚ dokumenty w celu uzupeÅ‚nienia wniosku kredytowego.',
    'docs_name'      => 'ImiÄ™ i nazwisko',
    'docs_email'     => 'Email',
    'docs_address'   => 'Adres',
    'docs_id_photo'  => 'Dokument toÅ¼samoÅ›ci',
    'docs_doc_type'  => 'Rodzaj dokumentu',
    'docs_recto'     => 'Strona przednia',
    'docs_verso'     => 'Strona tylna',
    'docs_success'   => 'Twoje dokumenty zostaÅ‚y wysÅ‚ane. Nasz zespÃ³Å‚ przejrzy je jak najszybciej.',

    'doc_type_id_card'   => 'DowÃ³d osobisty',
    'doc_type_passport'  => 'Paszport',
    'doc_type_license'   => 'Prawo jazdy',
    'doc_type_residence' => 'Karta pobytu',
    'doc_type_other'     => 'Inny dokument',

    'docs_confirm_subject'   => 'Twoje dokumenty zostaÅ‚y otrzymane',
    'docs_confirm_greeting'  => 'Witaj :name,',
    'docs_confirm_body'      => 'OtrzymaliÅ›my Twoje dokumenty (adres i dokument toÅ¼samoÅ›ci). Nasz zespÃ³Å‚ przejrzy je i skontaktuje siÄ™ z TobÄ… w ciÄ…gu 24 godzin.',
    'docs_confirm_footer'    => 'DziÄ™kujemy za zaufanie i pozostajemy do Twojej dyspozycji w razie pytaÅ„.',
    'docs_confirm_signature' => 'ZespÃ³Å‚ Credixa Invest',

    'docs_upload_hint'  => 'PrzeciÄ…gnij i upuÅ›Ä‡ lub kliknij, aby wybraÄ‡ plik',
    'docs_single_photo' => 'W przypadku tego rodzaju dokumentu wystarczy jedno zdjÄ™cie.',
];
