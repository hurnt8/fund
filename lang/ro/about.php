<?php

return [
    'banner_title' => 'Descoperă Aurenza Capital',
    'banner_subtitle' => 'Despre noi',
    'banner_description' => 'Împrumutați mai simplu cu soluții de credit flexibile și adaptate nevoilor dumneavoastră financiare.',

    'section1_title' => 'De ce să alegeți Aurenza Capital',
    'section1_p1' => 'La Aurenza Capital, credem cu tărie că fiecare persoană merită șansa de a-și îndeplini visele financiare. Încă de la înființare, ne-am angajat să oferim soluții de credit flexibile și accesibile pentru a răspunde nevoilor diverse ale clienților noștri. Fie că doriți să finanțați un proiect personal, să cumpărați o mașină sau să vă dezvoltați afacerea, avem creditul potrivit pentru dumneavoastră.',
    'section1_p2' => 'Serviciile noastre sunt concepute pentru a oferi o experiență fără bătăi de cap, cu un proces de cerere simplu și rapid. Înțelegem că fiecare client este unic, de aceea oferim credite personalizate care se adaptează nevoilor dumneavoastră specifice. În plus, consilierii noștri dedicați sunt mereu alături de dumneavoastră la fiecare etapă, asigurându-vă astfel un sprijin complet de la început până la sfârșit.',
    'proposed' => 'Vă propunem:',
    'check1' => 'Credite personalizate în funcție de nevoile dumneavoastră specifice.',
    'check2' => 'Consilieri dedicați pentru a vă ajuta la fiecare etapă a procesului.',
    'check3' => 'Condiții de rambursare flexibile, adaptate situației dumneavoastră financiare',
    'check4' => 'Credite fără a fi nevoie de fluturaș de salariu.',
    'section1_p3' => 'La Aurenza Capital, misiunea noastră este să vă ajutăm să vă atingeți obiectivele financiare prin soluții de credit personalizate. Alăturați-vă nouă chiar astăzi și descoperiți cum vă putem ajuta să vă concretizați proiectele.',
    'btn' => 'Găsiți creditul ideal',

    'faq_title' => 'Întrebări frecvente',
    'faq_description' => 'Găsiți răspunsuri la cele mai frecvente întrebări despre serviciile noastre de creditare.',
    'acc_1' => ' Ce tipuri de credite oferiți?',
    'acc_1_desc' => 'Oferim o varietate de credite, inclusiv credite personale, credite
    ipotecare, credite auto și credite pentru afaceri mici. Fiecare
    tip de credit este conceput pentru a răspunde unor nevoi specifice și oferă
    condiții flexibile.',
    'acc_2' => 'Cum pot depune o cerere de credit?',
    'acc_2_desc' => 'Puteți depune o cerere de credit direct pe site-ul nostru completând
    formularul de cerere online. Vă vom solicita
    informații de bază precum numele, datele de contact și suma
    de credit dorită. Consilierii noștri vă vor contacta pentru a finaliza cererea și
    a vă ghida pe tot parcursul procesului.',
    'acc_3' => 'Care sunt criteriile de eligibilitate pentru un credit?',
    'acc_3_desc' => 'Criteriile de eligibilitate variază în funcție de tipul de credit, dar în general
    trebuie să fiți major, să aveți un venit stabil și un scor de credit bun. De asemenea,
    vom analiza capacitatea dumneavoastră de rambursare și istoricul
    financiar.',
    'acc_4' => 'Care este termenul de procesare a unei cereri de credit?',
    'acc_4_desc' => '  Termenul de procesare variază în funcție de tipul de credit și complexitatea
    cererii. În general, cererile de credite personale sunt procesate în 24 până la 48
    de ore, în timp ce creditele ipotecare pot dura câteva zile din
    cauza verificărilor mai aprofundate necesare.',
    'acc_5' => 'Pot rambursa creditul în avans?',
    'acc_5_desc' => ' Da, puteți rambursa creditul în avans fără penalități. Vă
    încurajăm să rambursați creditul mai devreme dacă este posibil, deoarece acest lucru va reduce
    suma totală a dobânzilor pe care le veți plăti pe durata creditului.',

];
